...
from utils import stable_resize_token_embeddings, constants 
from models import zip_ 
import os 
...
def right_pad(inputs, target_size: Union[torch.Size, Sequence[int]], value=0.0):
...
def load_model_and_tokenizer_for_inference(
    ...
    """
    print(f"Loading model for inference: {model_name_or_path}")
    ...
    if isinstance(model, (transformers.OPTForCausalLM, transformers.LlamaForCausalLM)):
        ...
        if input_embedding_size != num_tokens and resize_token_embeddings_if_mismatch:
            ...
            stable_resize_token_embeddings(model, num_tokens)
...
def decode_prompts_with_huggingface_given_model(
    ...
    **decoding_kwargs,
    ...
    model.generate = cast_with_native_amp(model.generate, mixed_precision=mixed_precision)
    ...
    for batch_idx, start_idx in tqdm.tqdm(
        ...
        source = prepare_inputs(source, device=device)
        ...
        if (
            ...
            for internal_start_idx in range(
                ...
                internal_batch_sequences = right_pad(
            ...
            sequences = right_pad(sequences, (sequences.size(0), pad_to_length), value=tokenizer.pad_token_id)

        out_of_bound_mask = sequences >= len(tokenizer)
        if out_of_bound_mask.any():
            print(f"Found tokens outside the vocabulary: {sequences[out_of_bound_mask]}")
...
def decode_prompts_with_huggingface(
    ...
    **decoding_kwargs,
    ...
    model, tokenizer = load_model_and_tokenizer_for_inference(
        ...
        cache_dir=cache_dir,
        model_kwargs=dict(torch_dtype=convert_str_dtype_to_torch_dtype(mixed_precision)),
    )

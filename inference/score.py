...
from ..models import reward_model, zip_ 
from .decode import load_model_and_tokenizer_for_inference, cast_with_native_amp, is_main_process
from .decode import prepare_inputs, convert_str_dtype_to_torch_dtype 
...
def score_sequences_with_huggingface_given_model(
    ...
    model.forward = cast_with_native_amp(model.forward, mixed_precision=mixed_precision)
...
def score_sequences_with_huggingface(
    ...
    model, tokenizer = load_model_and_tokenizer_for_inference(
        ...
        cache_dir=cache_dir,
        model_kwargs=dict(
            torch_dtype=convert_str_dtype_to_torch_dtype(mixed_precision),
...
def rerank_sequences_with_huggingface(
    ...
    top_sequences = [[sequence[i] for i in top_index] for sequence, top_index in zip_(sequences, top_indices)]

...
def format_output(example: dict, eos_token: Optional[str] = None, output_key="output") -> str:
    if eos_token is None:
        eos_token = ""
    return f"{example['train'][output_key]}{eos_token}"
...
def preprocess_for_sft(
    ...
    examples = [s + t for s, t in zip_(sources, targets)] 
    # print(examples)
    examples_tokenized, sources_tokenized = [_tokenize_fn(strings, tokenizer) for strings in (examples, sources)]

    input_ids = examples_tokenized["input_ids"]
    labels = copy.deepcopy(input_ids)
    for label, source_len in zip_(labels, sources_tokenized["input_ids_lens"]):
        label[:source_len] = IGNORE_INDEX  # Input context should not contribute to loss.
...
def preprocess_for_reward_modeling(
    ...
    def _get_numeric_preference(example: dict):
        ...
        return {1: 0, 2: 1}[example['train']["preference"]]
    ...
    print(f"Tokenizing {len(list_dict_data)} pairs...")
    tokenized_0, tokenized_1 = tuple(_tokenize_fn(text_list, tokenizer) for text_list in (text_list_0, text_list_1))
    # "size" (bsz, 2, seq_len)
    input_ids = [list(pair) for pair in zip_(tokenized_0["input_ids"], tokenized_1["input_ids"])]
    labels = [list(pair) for pair in zip_(tokenized_0["labels"], tokenized_1["labels"])]
